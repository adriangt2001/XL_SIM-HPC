...
class WindowCrossAttn(nn.Module):
    def __init__(self, dim=180, num_heads=6, window_size=12, num_gs_seed=2304):
        ...
        indexed_coords_delta = list(map(list(set(indexed_coords_delta)).index, indexed_coords_delta))
...
class Fea2GS(nn.Module):
    def __init__(self, inchannel=64, outchannel=3, channel=180, num_heads=6, num_crossattn_blocks=1, num_crossattn_layers=2, num_selfattn_blocks = 6, num_selfattn_layers = 6,
        ...
        # GS RGB values
        self.mlp_block_light = nn.Sequential(
            nn.Linear(channel, channel),
            nn.ReLU(),
            nn.Linear(channel, channel * 4),
            nn.ReLU(),
            nn.Linear(channel * 4, int(outchannel * gs_up_factor))
        )
    ...
    def forward(self, srcs, scale):
        ...
        query_light = self.mlp_block_light(query).reshape(b, -1, self.outchannel)
        ...
        query = torch.cat([query_sigma, query_rho, query_alpha, query_mean, query_light],
